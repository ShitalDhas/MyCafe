...
import com.MyCafeManagement.POJO.Product;
...
import com.MyCafeManagement.service.ProductService;
		...
		try {
			if (jwtFilter.isAdmin()) {
				...
				if (product.isPresent()) {
					...
					return CafeUtils.getResponseEntity("Product with id " + id + " does not exists",
		...
		try {
			if (jwtFilter.isAdmin()) {
				...
				if (product.isPresent()) {
					...
					return CafeUtils.getResponseEntity("Product with id " + requestMap.get("id") + " does not exists",
							HttpStatus.NOT_FOUND);
			} else
				return CafeUtils.getResponseEntity(CafeConstants.UNAUTHORIZED_ACCESS, HttpStatus.UNAUTHORIZED);
		} catch (Exception exception) {
